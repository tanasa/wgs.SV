...
if (dim(hits)[1] == 0) { 
                          ...
                          write.table(message, file_exit, sep="\t", row.names=F, col.names = F, quote = FALSE)
                          stop("no breakpoints intersecting the genes", call=TRUE)
                       
} else {
                         message <- "there are breakpoints intersecting the GENES"
                         write.table(message, file_exit, sep="\t", row.names=F, col.names = F, quote = TRUE)
                         ## continue      
} 
...
### replacing vcf_PASS_COMBINED_FILTERS.df2 by vcf_PASS_COMBINED_FILTERS.df
...
for(id in ids)
{
 ...
 if(nrow(temp) == 4) 
 {
    ### considering simplifies annotations of the breaks :
    ### for INVERSIONS, considering BP3 and BP4
    ### as BP3 is external , 
    ### and BP4 is internal to the inversion .. 

    ### also, the reciprocal FUSION will be generated i.e. 
    ### the fusion between BP1 and BP2
    ### BP1 is internal, 
    ### while BP2 is external to the breakpoints ...

    ### in the dataframe TEMP, the BP are listed in the order 
    ### BP1, BP2, BP3, and BP4 

    {
         #####################################################################################  
         #####################################################################################
         ### here it is the possibility of another orientation : between the PARTNERS 1 and 2
         ...
         #####################################################################################  
         #####################################################################################
         ### here it is the possibility of an orientation : between the PARTNERS 3 and 4 
 
         if (  ( temp$couldBeThreePrimeStart[3] == "TRUE") & 
               ( temp$couldBeFivePrimeEnd[4] == "TRUE")  ) 
          {
               fusion.df <- data.frame( ID = unique(temp$vcfId[3], temp$vcfId[4]), 
                                        # Partner1 = temp$SYMBOL[1] , 
                                        # Partner2 = temp$SYMBOL[2] , 
                                          Partner1 = temp$SYMBOL[3] , 
                                          Partner2 = temp$SYMBOL[4] , 
                                        stringsAsFactors=FALSE 
                                       )
         ...
         #####################################################################################  
         #####################################################################################
         ### here it is the possibility of another orientation : between the PARTNERS 3 and 4
          
          if ( ( temp$couldBeThreePrimeStart[4] == "TRUE") & 
               ( temp$couldBeFivePrimeEnd[3] == "TRUE")  ) 
          {
               fusion.df <- data.frame( ID = unique(temp$vcfId[3], temp$vcfId[4]), 
                                        # Partner1 = temp$SYMBOL[2] , 
                                        # Partner2 = temp$SYMBOL[1] ,
                                        Partner1 = temp$SYMBOL[4] , 
                                        Partner2 = temp$SYMBOL[3] ,  
                                        stringsAsFactors=FALSE 
                                       )
